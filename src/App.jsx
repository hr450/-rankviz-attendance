...
function LoginFlow({ onLogin }) {
  ...
  async function handleIdSubmit() {
    setError('');
    try {
      const { needsSetup, name } = await checkFirstLogin(zkUserId);
      setEmpName(name);
      setStep(needsSetup ? 'setup' : 'login');
    } catch (e) {
  ...
  async function handleSetup() {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/employees?zk_user_id=eq.${zkUserId}`, {
        method: 'PATCH',
        headers: {
      ...
      onLogin({ zk_user_id: zkUserId, name: empName, email, role: 'employee' });
    } catch (e) {
  ...
  async function handleLogin() {
    setError('');
    try {
      const emp = await loginEmployee(zkUserId, email, password);
      onLogin(emp);
    } catch (e) {
  ...
  const boxStyle = { maxWidth: 340, margin: '80px auto', padding: 24, fontFamily: FONT_DISPLAY };
  const inputStyleLocal = { width: '100%', padding: 10, marginBottom: 10, border: '1px solid #ccc', borderRadius: 8, boxSizing: 'border-box' };
  const btnStyle = { width: '100%', padding: 10, background: COLORS.navy, color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer' };

  if (step === 'setup') {
    return (
      <div style={boxStyle}>
        <h2>Welcome, {empName}</h2>
        <p style={{ color: '#666', fontSize: 14 }}>First time here — set up your login.</p>
        <input style={inputStyleLocal} placeholder="Your Gmail" value={email} onChange={e => setEmail(e.target.value)} />
        <input style={inputStyleLocal} type="password" placeholder="Create Password" value={password} onChange={e => setPassword(e.target.value)} />
        <input style={inputStyleLocal} type="password" placeholder="Confirm Password" value={confirm} onChange={e => setConfirm(e.target.value)} />
        {error && <p style={{ color: 'red', fontSize: 13 }}>{error}</p>}
        <button style={btnStyle} onClick={handleSetup}>Set Password & Continue</button>
      </div>
    );
  }
  ...
  if (step === 'login') {
    return (
      <div style={boxStyle}>
        ...
        <input style={inputStyleLocal} placeholder="Gmail" value={email} onChange={e => setEmail(e.target.value)} />
        <input style={inputStyleLocal} type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} />
        {error && <p style={{ color: 'red', fontSize: 13 }}>{error}</p>}
  ...
  return (
    <div style={boxStyle}>
      ...
      <input style={inputStyleLocal} placeholder="Attendance ID" value={zkUserId} onChange={e => setZkUserId(e.target.value)} />
      {error && <p style={{ color: 'red', fontSize: 13 }}>{error}</p>}
      <button style={btnStyle} onClick={handleIdSubmit}>Continue</button>
    </div>
  );
}
